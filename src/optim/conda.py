...
class CondaProjector:
    ...
    def project_with_cached_ortho(self, input_matrix, svd_basis_matrix, step):
        ...
        return self.project(
            input_matrix, svd_basis_matrix, step, cached_ortho_matrix=self.ortho_matrix
        )
    ...
    def project_back(self, projected_matrix):
        ...
        if self.proj_type == "std":
            ...
            projected_back_matrix = torch.matmul(
                projected_back_matrix, right.to(device)
            )
        else:
            raise ValueError(f"Unknown proj_type: {self.proj_type}")
            ...
            for p in group["params"]:
                ...
                if "update_proj_gap" in group:
                    if "projector" not in state:
                        if group["dim"] <= 2:
                            state["projector"] = CondaProjector(
                                update_proj_gap=group["update_proj_gap"],
                                scale=group["scale"],
                                proj_type=group["proj_type"],
                            )
                    grad = state["projector"].project_with_cached_ortho(
                        grad, exp_avg, state["step"]
                    )
                    exp_avg = state["projector"].project_with_cached_ortho(
                        exp_avg, exp_avg, state["step"]
                    )
